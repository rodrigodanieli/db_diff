...
use Migration\Functions\Infrastructure\Repository;
...
class Diff
{
    ...
    private GetFunctionsFromBase $get_functions_from_base;
    ...
    public function __construct()
    {
        ...
        $this->get_functions_from_base = new GetFunctionsFromBase(new Repository);
    }
    ...
    private function getBasesFunctions($inverse = false)
    {
        ...
        $this->logger->info("Getting Bases Functions");
        ...
        foreach ($bases as $base) {
            try {
                ...
                $functions = $this->get_functions_from_base->execute($function_dto);
                foreach ($functions as $proc) {
                    $this->functions[] = [
                        "base" => $base,
                        "function" => $proc
                    ];
    ...
    private function process($inverse = false): array
    {
        ...
        if ($this->workers > 1) {
            $script = [];
            $chunk = array_chunk($this->functions, (int)ceil(count($this->functions) / $this->workers));
            $runtimes = [];
            $futures = [];
            ...
            return json_decode(self::diff($this->project, $base, $compare, $this->functions), true);
        }
    }
    ...
    private function generate($data, $folder = "structure")
    {
        foreach ($data as $base => $scr) {
            ...
            $fscript .= $scr;
            file_put_contents("$path/functions.sql", $fscript);
        }
    }
}
