...
use Migration\Trigger\Infrastructure\Repository;
...
class Diff
{
    ...
    private GetTriggersFromBase $get_trigger_from_base;
    private Compare $compare_trigger;
    ...
    public function __construct()
    {
        ...
        $this->get_trigger_from_base = new GetTriggersFromBase(new Repository);
    }
    ...
    private function getBasesTrigger($inverse = false)
    {
        ...
        $this->logger->info("Getting Bases Trigger");
        ...
        foreach ($bases as $base) {
            try {
                ...
                $trigger = $this->get_trigger_from_base->execute($trigger_dto);
                foreach ($trigger as $proc) {
                    $this->trigger[] = [
                        "base" => $base,
                        "trigger" => $proc
                    ];
    ...
    private function process($inverse = false): array
    {
        ...
        if ($this->workers > 1) {
            $script = [];
            $chunk = array_chunk($this->trigger, (int)ceil(count($this->trigger) / $this->workers));
            $runtimes = [];
            $futures = [];
            ...
            return json_decode(self::diff($this->project, $base, $compare, $this->trigger), true);
        }
    }
    ...
    private static function diff($project, $base_1, $base_2, $data, $worker = "")
    {
        ...
        $compare_trigger = new Compare(new Repository);
        $count = 0;
        $total = count($data);
        foreach ($data as $info) {
            try {
                ...
                $logger->debug("Process $base $trigger ($count/$total)");
                $compare_trigger_dto = new CompareDto($base, $trigger, $database_reference, $database_to_apply);
                $value = $compare_trigger->execute($compare_trigger_dto);
    ...
    private function generate($data, $folder = "structure")
    {
        foreach ($data as $base => $scr) {
            ...
            $fscript .= $scr;
            file_put_contents("$path/trigger.sql", $fscript);
        }
    }
}
